...
class Platform():
    ...
    def feedback(self, observe, reservation_value, speed, current_order, current_order_num, assignment, new_orders_state, price_mu, price_sigma, reward_func, punish_rate, threshold, current_time, worker_Q_Net, exploration_rate, worker_reject_punishment, device, worker_state, time_prediction):
        feedback_table = []
        new_route_table = []
        ...
        results = Parallel(n_jobs=self.njobs)(
            delayed(excute)(observe[i], reservation_value[i], speed[i], current_order[i], current_order_num[i], assignment[i], new_orders_state, price_mu[i], price_sigma[i], reward_func, punish_rate, threshold, current_time, worker_Q_Net, exploration_rate, worker_reject_punishment, device, worker_state[i], self.probability_worker, time_prediction[i])
            for i
...
'''
def excute(observe, reservation_value, speed, current_order, current_order_num, assignment, new_orders_state, price_mu_vector, price_sigma_vector, reward_func, punish_rate, threshold, current_time, worker_Q_Net, exploration_rate, worker_reject_punishment, device, worker_state, probability_worker, time_prediction):
    ...
    if assignment is not None and observe[3] == 0:
        ...
        time_prediction = time_prediction[assignment]
        ...
        if flag: #accept
            ...
            if len(new_route)!=0:
                ...
                if worker_Q_Net is not None:
                    salary = (work_add + time_prediction * speed) * price
                else:
                    salary = work_add * price

                reward = reward_func(time_add / speed, timeout, salary, direct_distance, price)
                feedback = [[observe,current_order,current_order_num,new_orders_state[assignment], current_time], [price,price_log_prop,work_add,salary,time_prediction], reward, pickup_time[0]]
